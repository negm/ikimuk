...
$submissions_view = NULL; // Initialize page object first

class csubmissions_view extends csubmissions {
	...
	// Page object name
	var $PageObjName = 'submissions_view';
	...
	function Page_Main() {
		...
		if ($this->IsPageRequest()) { // Validate request
			if (@$_GET["id"] <> "") {
				...
				$sReturnUrl = "submissionslist.php"; // Return to list
			}
			...
			switch ($this->CurrentAction) {
				case "I": // Get a record to display
					if (!$this->LoadRow()) { // Load record based on key
						...
						$sReturnUrl = "submissionslist.php"; // No matching record, return to list
					}
			}
			...
			$sReturnUrl = "submissionslist.php"; // Not page request, return to list
		}
		if ($sReturnUrl <> "")
	...
	function SetupExportOptions() {
		...
		$item->Body = "<a name=\"emf_submissions\" id=\"emf_submissions\" href=\"javascript:void(0);\" onclick=\"ew_EmailDialogShow({lnk:'emf_submissions',hdr:ewLanguage.Phrase('ExportToEmail'),key:" . ew_ArrayToJsonAttr($this->RecKey) . ",sel:false});\">" . $Language->Phrase("ExportToEmail") . "</a>";
		$item->Visible = FALSE;
...
// Create page object
if (!isset($submissions_view)) $submissions_view = new csubmissions_view();

// Page init
$submissions_view->Page_Init();

// Page main
$submissions_view->Page_Main();
?>
...
// Page object
var submissions_view = new ew_Page("submissions_view");
submissions_view.PageID = "view"; // Page ID
var EW_PAGE_ID = submissions_view.PageID; // For backward compatibility

// Form object
var fsubmissionsview = new ew_Form("fsubmissionsview");

// Form_CustomValidate event
fsubmissionsview.Form_CustomValidate =  
...
fsubmissionsview.ValidateRequired = true;
<?php } else { ?>
fsubmissionsview.ValidateRequired = false; 
<?php } ?>
...
<?php } ?>
<p><span class="ewTitle ewTableTitle"><?php echo $Language->Phrase("View") ?>&nbsp;<?php echo $Language->Phrase("TblTypeTABLE") ?><?php echo $submissions->TableCaption() ?>&nbsp;&nbsp;</span><?php $submissions_view->ExportOptions->Render("body"); ?>
</p>
<?php if ($submissions->Export == "") { ?>
<p class="phpmaker">
<a href="<?php echo $submissions_view->ListUrl ?>"><?php echo $Language->Phrase("BackToList") ?></a>&nbsp;
<?php if ($Security->IsLoggedIn()) { ?>
<?php if ($submissions_view->AddUrl <> "") { ?>
<a href="<?php echo $submissions_view->AddUrl ?>"><?php echo $Language->Phrase("ViewPageAddLink") ?></a>&nbsp;
<?php } ?>
<?php } ?>
<?php if ($Security->IsLoggedIn()) { ?>
<?php if ($submissions_view->EditUrl <> "") { ?>
<a href="<?php echo $submissions_view->EditUrl ?>"><?php echo $Language->Phrase("ViewPageEditLink") ?></a>&nbsp;
<?php } ?>
<?php } ?>
<?php if ($Security->IsLoggedIn()) { ?>
<?php if ($submissions_view->CopyUrl <> "") { ?>
<a href="<?php echo $submissions_view->CopyUrl ?>"><?php echo $Language->Phrase("ViewPageCopyLink") ?></a>&nbsp;
<?php } ?>
<?php } ?>
</p>
<?php } ?>
<?php $submissions_view->ShowPageHeader(); ?>
<?php
$submissions_view->ShowMessage();
?>
<form name="fsubmissionsview" id="fsubmissionsview" class="ewForm" action="" method="post">
<input type="hidden" name="t" value="submissions">
<table cellspacing="0" class="ewGrid"><tr><td class="ewGridContent">
<div class="ewGridMiddlePanel">
<table cellspacing="0" id="tbl_submissionsview" class="ewTable">
...
<p>
<script type="text/javascript">
fsubmissionsview.Init();
</script>
<?php
$submissions_view->ShowPageFooter();
if (EW_DEBUG_ENABLED)
	echo ew_DebugMsg();
?>
<?php if ($submissions->Export == "") { ?>
...
<?php
$submissions_view->Page_Terminate();
?>
