...
$ip2nation_countries_add = NULL; // Initialize page object first

class cip2nation_countries_add extends cip2nation_countries {
	...
	// Page object name
	var $PageObjName = 'ip2nation_countries_add';
	...
	function Page_Main() {
		...
		if (@$_POST["a_add"] <> "") {
			...
			if (@$_GET["country_code"] != "") {
				...
				$this->setKey("country_code", $this->country_code->CurrentValue); // Set up key
			} else {
				$this->setKey("country_code", ""); // Clear key
		...
		switch ($this->CurrentAction) {
			...
			case "A": // Add new record
				...
				if ($this->AddRow($this->OldRecordset)) { // Add successful
					...
					if (ew_GetPageName($sReturnUrl) == "ip2nation_countriesview.php")
	...
	function LoadDefaultValues() {
		...
		$this->iso_code_2->CurrentValue = NULL;
		$this->iso_code_2->OldValue = $this->iso_code_2->CurrentValue;
		$this->iso_code_3->CurrentValue = NULL;
		$this->iso_code_3->OldValue = $this->iso_code_3->CurrentValue;
		$this->iso_country->CurrentValue = NULL;
		$this->iso_country->OldValue = $this->iso_country->CurrentValue;
		$this->country_name->CurrentValue = NULL;
		$this->country_name->OldValue = $this->country_name->CurrentValue;
		$this->delivery_charge->CurrentValue = NULL;
		$this->delivery_charge->OldValue = $this->delivery_charge->CurrentValue;
		$this->phone_code->CurrentValue = NULL;
		$this->phone_code->OldValue = $this->phone_code->CurrentValue;
		$this->lat->CurrentValue = 0;
		$this->lon->CurrentValue = 0;
	}
	...
	function AddRow($rsold = NULL) {
		...
		// country_code
		$this->country_code->SetDbValueDef($rsnew, $this->country_code->CurrentValue, "", FALSE);

		// iso_code_2
		$this->iso_code_2->SetDbValueDef($rsnew, $this->iso_code_2->CurrentValue, "", FALSE);

		// iso_code_3
		$this->iso_code_3->SetDbValueDef($rsnew, $this->iso_code_3->CurrentValue, NULL, FALSE);

		// iso_country
		$this->iso_country->SetDbValueDef($rsnew, $this->iso_country->CurrentValue, "", FALSE);

		// country_name
		$this->country_name->SetDbValueDef($rsnew, $this->country_name->CurrentValue, "", FALSE);

		// delivery_charge
		$this->delivery_charge->SetDbValueDef($rsnew, $this->delivery_charge->CurrentValue, 0, FALSE);

		// phone_code
		$this->phone_code->SetDbValueDef($rsnew, $this->phone_code->CurrentValue, 0, FALSE);

		// lat
		$this->lat->SetDbValueDef($rsnew, $this->lat->CurrentValue, 0, strval($this->lat->CurrentValue) == "");

		// lon
		$this->lon->SetDbValueDef($rsnew, $this->lon->CurrentValue, 0, strval($this->lon->CurrentValue) == "");
		...
		if ($bInsertRow && $this->ValidateKey && $this->country_code->CurrentValue == "" && $this->country_code->getSessionValue() == "") {
...
// Create page object
if (!isset($ip2nation_countries_add)) $ip2nation_countries_add = new cip2nation_countries_add();

// Page init
$ip2nation_countries_add->Page_Init();

// Page main
$ip2nation_countries_add->Page_Main();
?>
<?php include_once "header.php" ?>
<script type="text/javascript">

// Page object
var ip2nation_countries_add = new ew_Page("ip2nation_countries_add");
ip2nation_countries_add.PageID = "add"; // Page ID
var EW_PAGE_ID = ip2nation_countries_add.PageID; // For backward compatibility

// Form object
var fip2nation_countriesadd = new ew_Form("fip2nation_countriesadd");

// Validate form
fip2nation_countriesadd.Validate = function(fobj) {
...
fip2nation_countriesadd.Form_CustomValidate =  
...
fip2nation_countriesadd.ValidateRequired = true;
<?php } else { ?>
fip2nation_countriesadd.ValidateRequired = false; 
<?php } ?>
...
<p><span class="ewTitle ewTableTitle"><?php echo $Language->Phrase("Add") ?>&nbsp;<?php echo $Language->Phrase("TblTypeTABLE") ?><?php echo $ip2nation_countries->TableCaption() ?></span></p>
<p class="phpmaker"><a href="<?php echo $ip2nation_countries->getReturnUrl() ?>"><?php echo $Language->Phrase("GoBack") ?></a></p>
<?php $ip2nation_countries_add->ShowPageHeader(); ?>
<?php
$ip2nation_countries_add->ShowMessage();
?>
<form name="fip2nation_countriesadd" id="fip2nation_countriesadd" class="ewForm" action="<?php echo ew_CurrentPage() ?>" method="post" onsubmit="return ewForms[this.id].Submit();">
<p>
<input type="hidden" name="t" value="ip2nation_countries">
<input type="hidden" name="a_add" id="a_add" value="A">
<table cellspacing="0" class="ewGrid"><tr><td class="ewGridContent">
<div class="ewGridMiddlePanel">
<table cellspacing="0" id="tbl_ip2nation_countriesadd" class="ewTable">
...
<input type="text" name="x_country_code" id="x_country_code" size="30" maxlength="4" value="<?php echo $ip2nation_countries->country_code->EditValue ?>"<?php echo $ip2nation_countries->country_code->EditAttributes() ?>>
...
<script type="text/javascript">
fip2nation_countriesadd.Init();
</script>
<?php
$ip2nation_countries_add->ShowPageFooter();
if (EW_DEBUG_ENABLED)
	echo ew_DebugMsg();
?>
<script type="text/javascript">
...
<?php
$ip2nation_countries_add->Page_Terminate();
?>
