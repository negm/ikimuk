...
//
// Table class for message_log
//
class cmessage_log extends cTable {
	var $id;
	var $messageId;
	var $requestId;
	var $recepient;
	...
	function __construct() {
		...
		$this->TableVar = 'message_log';
		$this->TableName = 'message_log';
		...
		// id
		$this->id = new cField('message_log', 'message_log', 'x_id', 'id', '`id`', '`id`', 3, -1, FALSE, '`id`', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
		$this->id->FldDefaultErrMsg = $Language->Phrase("IncorrectInteger");
		$this->fields['id'] = &$this->id;

		// messageId
		$this->messageId = new cField('message_log', 'message_log', 'x_messageId', 'messageId', '`messageId`', '`messageId`', 200, -1, FALSE, '`messageId`', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
		$this->fields['messageId'] = &$this->messageId;

		// requestId
		$this->requestId = new cField('message_log', 'message_log', 'x_requestId', 'requestId', '`requestId`', '`requestId`', 200, -1, FALSE, '`requestId`', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
		$this->fields['requestId'] = &$this->requestId;

		// recepient
		$this->recepient = new cField('message_log', 'message_log', 'x_recepient', 'recepient', '`recepient`', '`recepient`', 200, -1, FALSE, '`recepient`', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
		$this->fields['recepient'] = &$this->recepient;

		// subject
		$this->subject = new cField('message_log', 'message_log', 'x_subject', 'subject', '`subject`', '`subject`', 200, -1, FALSE, '`subject`', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
		$this->fields['subject'] = &$this->subject;

		// body
		$this->body = new cField('message_log', 'message_log', 'x_body', 'body', '`body`', '`body`', 200, -1, FALSE, '`body`', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
		$this->fields['body'] = &$this->body;

		// date_added
		$this->date_added = new cField('message_log', 'message_log', 'x_date_added', 'date_added', '`date_added`', 'DATE_FORMAT(`date_added`, \'%d/%m/%Y %H:%i:%s\')', 135, 7, FALSE, '`date_added`', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
	...
	// View URL
	function GetViewUrl() {
		return $this->KeyUrl("message_logview.php", $this->UrlParm());
	}

	// Add URL
	function GetAddUrl() {
		return "message_logadd.php";
	}

	// Edit URL
	function GetEditUrl($parm = "") {
		return $this->KeyUrl("message_logedit.php", $this->UrlParm($parm));
	}
	...
	function GetCopyUrl($parm = "") {
		return $this->KeyUrl("message_logadd.php", $this->UrlParm($parm));
	}
	...
	function GetDeleteUrl() {
		return $this->KeyUrl("message_logdelete.php", $this->UrlParm());
	}
	...
	function ExportDocument(&$Doc, &$Recordset, $StartRec, $StopRec, $ExportPageType = "") {
		...
		if ($Doc->Horizontal) { // Horizontal format, write header
			...
			if ($ExportPageType == "view") {
				...
				$Doc->ExportCaption($this->requestId);
				$Doc->ExportCaption($this->recepient);
		...
		while (!$Recordset->EOF && $RecCnt < $StopRec) {
			...
			if (intval($RecCnt) >= intval($StartRec)) {
				...
				if ($ExportPageType == "view") {
					...
					$Doc->ExportField($this->requestId);
					$Doc->ExportField($this->recepient);
