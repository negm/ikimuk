...
$competition_add = NULL; // Initialize page object first

class ccompetition_add extends ccompetition {
	...
	// Page object name
	var $PageObjName = 'competition_add';
		...
		switch ($this->CurrentAction) {
			...
			case "A": // Add new record
				...
				if ($this->AddRow($this->OldRecordset)) { // Add successful
					...
					if (ew_GetPageName($sReturnUrl) == "competitionview.php")
	...
	function LoadDefaultValues() {
		...
		$this->competition_header->CurrentValue = NULL;
		$this->competition_header->OldValue = $this->competition_header->CurrentValue;
		$this->submission_header->CurrentValue = NULL;
		$this->submission_header->OldValue = $this->submission_header->CurrentValue;
		$this->competition_order->CurrentValue = NULL;
		$this->competition_order->OldValue = $this->competition_order->CurrentValue;
		$this->start_date->CurrentValue = "0000-00-00 00:00:00";
		$this->end_date->CurrentValue = "0000-00-00 00:00:00";
		$this->submission_deadline->CurrentValue = "0000-00-00 00:00:00";
	...
	function AddRow($rsold = NULL) {
		...
		// competition_header
		$this->competition_header->SetDbValueDef($rsnew, $this->competition_header->CurrentValue, NULL, FALSE);

		// submission_header
		$this->submission_header->SetDbValueDef($rsnew, $this->submission_header->CurrentValue, NULL, FALSE);

		// competition_order
		$this->competition_order->SetDbValueDef($rsnew, $this->competition_order->CurrentValue, NULL, FALSE);

		// start_date
		$this->start_date->SetDbValueDef($rsnew, ew_UnFormatDateTime($this->start_date->CurrentValue, 7), ew_CurrentDate(), strval($this->start_date->CurrentValue) == "");

		// end_date
		$this->end_date->SetDbValueDef($rsnew, ew_UnFormatDateTime($this->end_date->CurrentValue, 7), ew_CurrentDate(), strval($this->end_date->CurrentValue) == "");

		// submission_deadline
		$this->submission_deadline->SetDbValueDef($rsnew, ew_UnFormatDateTime($this->submission_deadline->CurrentValue, 7), ew_CurrentDate(), strval($this->submission_deadline->CurrentValue) == "");
...
// Create page object
if (!isset($competition_add)) $competition_add = new ccompetition_add();

// Page init
$competition_add->Page_Init();

// Page main
$competition_add->Page_Main();
?>
<?php include_once "header.php" ?>
<script type="text/javascript">

// Page object
var competition_add = new ew_Page("competition_add");
competition_add.PageID = "add"; // Page ID
var EW_PAGE_ID = competition_add.PageID; // For backward compatibility

// Form object
var fcompetitionadd = new ew_Form("fcompetitionadd");

// Validate form
fcompetitionadd.Validate = function(fobj) {
...
fcompetitionadd.Form_CustomValidate =  
...
fcompetitionadd.ValidateRequired = true;
<?php } else { ?>
fcompetitionadd.ValidateRequired = false; 
<?php } ?>
...
<p><span class="ewTitle ewTableTitle"><?php echo $Language->Phrase("Add") ?>&nbsp;<?php echo $Language->Phrase("TblTypeTABLE") ?><?php echo $competition->TableCaption() ?></span></p>
<p class="phpmaker"><a href="<?php echo $competition->getReturnUrl() ?>"><?php echo $Language->Phrase("GoBack") ?></a></p>
<?php $competition_add->ShowPageHeader(); ?>
<?php
$competition_add->ShowMessage();
?>
<form name="fcompetitionadd" id="fcompetitionadd" class="ewForm" action="<?php echo ew_CurrentPage() ?>" method="post" onsubmit="return ewForms[this.id].Submit();">
<p>
<input type="hidden" name="t" value="competition">
<input type="hidden" name="a_add" id="a_add" value="A">
<table cellspacing="0" class="ewGrid"><tr><td class="ewGridContent">
<div class="ewGridMiddlePanel">
<table cellspacing="0" id="tbl_competitionadd" class="ewTable">
...
<script type="text/javascript">
fcompetitionadd.Init();
</script>
<?php
$competition_add->ShowPageFooter();
if (EW_DEBUG_ENABLED)
	echo ew_DebugMsg();
?>
<script type="text/javascript">
...
<?php
$competition_add->Page_Terminate();
?>
