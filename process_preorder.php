...
if (!$param["address"] || strlen(trim($param["address"])) < 9 )
{$error = 'address error'; }
$preorder->address = $param["address"];
if (!isset($param["region"]))
{return 'region error';}
...
if (count(details) < 1)
{
    ...
    if (!$error)
    {
     foreach ($details as $key=>$detail )
    {
         $preorder_details->size = $size_enum[$$detail[0]];
         $preorder_details->cut = $cut_enum[$$detail[1]];
         $preorder_details->preorder_id = $preorder->id;
         $preorder_details->price = $product->price;
         $preorder_details->quantity = 1;
         $preorder_details->insert();
    }
    }
}
//if ($preorder->alreadyPreordered())
