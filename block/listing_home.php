...
echo '<div class="container compHeader center"><div class="row"><img  class="span12" src="/img/header_comp_ikimuk_twisted.jpg" alt="competition header ikimuk"/></div></div>';
echo '<div class="container center"><b class="tlblue tlarge">OUR THIRD COMPETITION </b> <b class="tpink tmedium">  (ends on 21/01/2013</b>)<div class="lineb"></div><br>';
$count = 0;
...
{   $daysLeft = floor((strtotime($row["end_date"]) - time())/(60*60*24));
    ...
    echo '<div class="span4 wrapper" itemscope itemtype="http://schema.org/CreativeWork" ><a itemprop="url" class="home_list" href="/design/'.$row["id"].'/'.str_replace(".","",str_replace(" ","-",trim($row["title"]))).'" >
            <div class="caption"><b>PREORDER NOW</b></div><img itemprop="image" class="" src="'.$row["url"].'" alt="'.$row["title"].' ikimuk"/></a>';
